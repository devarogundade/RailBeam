...
export async function createApiClient(): Promise<AxiosInstance> {
  ...
  if (!walletClient || !account) return base;
...
export class ClientApi {
  ...
  async getVirtualCard(params?: { refresh?: boolean }): Promise<{ card: VirtualCardSummary | null }> {
    const client = await createApiClient();
  ...
  async ensureVirtualCard(body?: CreateVirtualCardBody): Promise<{ card: VirtualCardSummary }> {
    const client = await createApiClient();
    const response = await client.post(`/issuing/virtual-card`, body ?? {});
    return response.data;
  }
