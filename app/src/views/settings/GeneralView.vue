...
const saveChanges = async () => {
    ...
    let metadata_value = Converter.parseMerchantMetadata(walletStore.merchant.metadata_value);
    metadata_value = {
        ...metadata_value,
        name: name.value,
...
const syncMerchantFields = () => {
    if (!walletStore.merchant) return;
    const m = Converter.parseMerchantMetadata(walletStore.merchant.metadata_value);
    name.value = typeof m.name === 'string' ? m.name : null;
    imageURL.value = typeof m.imageURL === 'string' ? m.imageURL : null;
};
