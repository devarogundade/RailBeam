...
function createBeamAuthApi(http2, setAccessToken) {
  ...
  const me = async () => http2.requestJson("GET", "/auth/me", { parse: authMeResponseSchema });
...
function createBeamHandlersApi(http2) {
  return {
    list: () => http2.requestJson("GET", "/handlers", { parse: handlersListResponseSchema }),
...
function createBeamUsersApi(http2) {
  return {
    ...
    uploadFile: (file) => {
      ...
      return http2.requestFormData("POST", "/users/me/files", fd, userUploadResultSchema);
    },
...
var identityRegistryReadAbi = parseAbi([
...
var identityRegistryWriteAbi = parseAbi([
...
var reputationRegistryReadAbi = parseAbi([
  "function getSummary(uint256 agentId,address[] clientAddresses,string tag1,string tag2) view returns (uint64 count,int128 summaryValue,uint8 summaryValueDecimals)",
  "function getLastIndex(uint256 agentId,address clientAddress) view returns (uint64)"
]);
var reputationRegistryWriteAbi = parseAbi([
...
var validationRegistryReadAbi = parseAbi([
...
var validationRegistryWriteAbi = parseAbi([
...
var BeamChainModule = class {
  ...
  constructor(runtime) {
    ...
    this.read = {
      identity: {
        ownerOf: (tokenId) => readContract(pc, {
          address: id,
          abi: identityRegistryReadAbi,
          functionName: "ownerOf",
          args: [tokenId]
        }),
        tokenURI: (tokenId) => readContract(pc, {
          address: id,
          abi: identityRegistryReadAbi,
          functionName: "tokenURI",
          args: [tokenId]
        }),
        getAgentWallet: (agentId) => readContract(pc, {
          address: id,
          abi: identityRegistryReadAbi,
          functionName: "getAgentWallet",
          args: [agentId]
        }),
        getFees: (agentId) => readContract(pc, {
          address: id,
          abi: identityRegistryReadAbi,
          functionName: "getFees",
          args: [agentId]
        }),
        getMetadata: (agentId, metadataKey) => readContract(pc, {
          address: id,
          abi: identityRegistryReadAbi,
      ...
      reputation: {
        ...
        getLastIndex: (agentId, clientAddress) => readContract(pc, {
          address: rep,
          abi: reputationRegistryReadAbi,
  ...
  forAccount(account) {
    ...
    return {
      identity: {
        register: () => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
          functionName: "register",
          args: []
        }),
        registerWithUri: (agentURI) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
          functionName: "register",
          args: [agentURI]
        }),
        registerWithMetadataAndFees: (agentURI, metadata, feesPerDay) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
        ...
        setAgentURI: (agentId, newURI) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
          functionName: "setAgentURI",
          args: [agentId, newURI]
        }),
        setMetadata: (agentId, metadataKey, metadataValue) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
          functionName: "setMetadata",
          args: [agentId, metadataKey, metadataValue]
        }),
        unsetAgentWallet: (agentId) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
          functionName: "unsetAgentWallet",
          args: [agentId]
        }),
        setAgentWallet: (agentId, newWallet, deadline, signature) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
          functionName: "setAgentWallet",
          args: [agentId, newWallet, deadline, signature]
        }),
        transfer: (from, to, tokenId, sealedKey, proof) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
          functionName: "transfer",
          args: [from, to, tokenId, sealedKey, proof]
        }),
        clone: (to, tokenId, sealedKey, proof) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
          functionName: "clone",
          args: [to, tokenId, sealedKey, proof]
        }),
        subscribe: (agentId, numDays, opts) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
        ...
        unsubscribe: (agentId) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
          functionName: "unsubscribe",
          args: [agentId]
        }),
        setFees: (agentId, feePerDay) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
          functionName: "setFees",
          args: [agentId, feePerDay]
        }),
        transferFrom: (from, to, tokenId) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
          functionName: "transferFrom",
          args: [from, to, tokenId]
        }),
        safeTransferFrom: (from, to, tokenId) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
          functionName: "safeTransferFrom",
          args: [from, to, tokenId]
        }),
        approve: (to, tokenId) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
          functionName: "approve",
          args: [to, tokenId]
        }),
        setApprovalForAll: (operator, approved) => writeContract(walletClient, {
          address: id,
          abi: identityRegistryWriteAbi,
      ...
      reputation: {
        giveFeedback: (a) => writeContract(walletClient, {
          address: rep,
        ...
        revokeFeedback: (agentId, feedbackIndex) => writeContract(walletClient, {
          address: rep,
          abi: reputationRegistryWriteAbi,
          functionName: "revokeFeedback",
          args: [agentId, feedbackIndex]
        }),
        appendResponse: (agentId, clientAddress, feedbackIndex, responseURI, responseHash) => writeContract(walletClient, {
          address: rep,
          abi: reputationRegistryWriteAbi,
      ...
      validation: {
        validationRequest: (validatorAddress, agentId, requestURI, requestHash) => writeContract(walletClient, {
          address: val,
          abi: validationRegistryWriteAbi,
          functionName: "validationRequest",
          args: [validatorAddress, agentId, requestURI, requestHash]
        }),
        validationResponse: (requestHash, response, responseURI, responseHash, tag) => writeContract(walletClient, {
          address: val,
          abi: validationRegistryWriteAbi,
