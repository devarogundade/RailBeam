...
export abstract class BaseTransaction {
  ...
  private readonly transactionOrigin: string;
  ...
  constructor(client: BeamClient) {
    ...
    this.transactionOrigin = new URL(this.transactionURL).origin;
  }
  ...
  protected async launchTabAndAwaitResult(
    ...
    setTimeout(() => {
      ...
      this.currentTab.postMessage(JSON.stringify(data), this.transactionOrigin);
    }, 3000);
  ...
  private messageHandler(event: MessageEvent, callback: (data: any) => void) {
    ...
    if (event.origin !== this.transactionOrigin) {
